...
import { CategoryItem, PagedAudit, PagedProducts, PermissionItem, ProductItem, RoleItem, SaveCategoryRequest, SaveProductRequest, SaveRoleRequest, SaveUserRequest, UserItem } from './management.models';
...
export class ManagementApiService {
  ...
  audit(filters: Record<string, string | number>) { let params = new HttpParams(); Object.entries(filters).filter(([, value]) => value !== '').forEach(([key, value]) => params = params.set(key, value)); return this.http.get<PagedAudit>('/api/audit', { params }); }
  ...
  products(filters: { search?: string; categoryId?: string; isActive?: string; page?: number; pageSize?: number }) { let params = new HttpParams(); Object.entries(filters).filter(([, value]) => value !== undefined && value !== '').forEach(([key, value]) => params = params.set(key, value!)); return this.http.get<PagedProducts>('/api/products', { params }); }
